...
function BicubicSplineInterpolation(M, cols, rows, clamp) {
  ...
  this.interpolate = function (x, y) {
    ...
    let c = [this.spline(this.p0, C), this.spline(this.p1, C), this.spline(this.p2, C), this.spline(this.p3, C)];
...
EphemUtils.julianDate = function (t) {
  ...
  let dayf =
    (t.getUTCHours() + (t.getUTCMinutes() + (t.getUTCSeconds() + t.getUTCMilliseconds() / 1000) / 60) / 60) / 24;
...
EphemUtils.deltaAT_data = [
  ...
  [2437665.5, 1.845858, 37665, 0.0011232],
  [2438334.5, 1.945858, 37665, 0.0011232],
  [2438395.5, 3.24013, 38761, 0.001296],
  [2438486.5, 3.34013, 38761, 0.001296],
  [2438639.5, 3.44013, 38761, 0.001296],
  [2438761.5, 3.54013, 38761, 0.001296],
  [2438820.5, 3.64013, 38761, 0.001296],
  [2438942.5, 3.74013, 38761, 0.001296],
  [2439004.5, 3.84013, 38761, 0.001296],
  [2439126.5, 4.31317, 39126, 0.002592],
  [2439887.5, 4.21317, 39126, 0.002592],
  [2441317.5, 10.0],
  [2441499.5, 11.0],
  [2441683.5, 12.0],
  [2442048.5, 13.0],
...
EphemUtils.obliquity = function (t) {
  ...
  let T4 = T3 * T;
  let T5 = T4 * T;
  return EphemUtils.degreesToRadians(
    (84381.406 - T * 46.836769 - T2 * 0.0001831 + T3 * 0.0020034 - T4 * 0.000000576 - T5 * 0.0000000434) / 3600
  );
};
...
EphemUtils.sphericalToRectangular = function (s) {
  ...
  return { x: clon * clat, y: slon * clat, z: slat };
};
...
EphemUtils.rectangularToSpherical = function (r) {
  ...
  return { lon: m2 == 0 ? 0 : Math.atan2(r.y, r.x), lat: r.z == 0 ? 0 : Math.atan2(r.z, Math.sqrt(m2)) };
};
...
EphemUtils.rectangularEquatorialToEcliptic = function (r, se, ce) {
  return { x: r.x, y: r.y * ce + r.z * se, z: r.z * ce - r.y * se };
};
...
EphemUtils.rectangularEquatorialToGalactic = function (r) {
  return {
    ...
    y: -0.872844082054 * r.x - 0.48492863607 * r.y + 0.746511167077 * r.z,
...
function CoordinateInterpolation(Ma, Md, x0, y0, x1, y1, delta, date, scale) {
  ...
  if (this.rows < 2 || this.cols < 2) throw new Error("CoordinateInterpolation: Insufficient interpolation space.");
  ...
  this.interpolate = function (x, y, withGalactic, withEcliptic) {
    ...
    if (withGalactic || (withEcliptic && this.date !== undefined)) {
      let s = { lon: EphemUtils.degreesToRadians(alpha), lat: EphemUtils.degreesToRadians(delta) };
      let r = EphemUtils.sphericalToRectangular(s);
      if (withGalactic) {
        let g = EphemUtils.rectangularToSphericalDegreesConstrained(EphemUtils.rectangularEquatorialToGalactic(r));
        retVal.l = g.lon;
        retVal.b = g.lat;
      }
      if (withEcliptic)
  ...
  this.interpolateAsText = function (x, y, units, withGalactic, withEcliptic) {
    if (units === undefined) units = true;
    let q = this.interpolate(x, y, withGalactic, withEcliptic);
    let retVal = {
      alpha: this.angleString(q.alpha / 15, 24 /*range*/, false /*sign*/, this.precision + 1, units),
  ...
  this.angleString = function (angle, range, sign, precision, units) {
    ...
    let result =
      (sign ? (d[0] < 0 ? "-" : "+") : "") + whitePadded(dd, dw) + du + zeroPadded(mm, 2) + mu + zeroPadded(ss, 2);
    if (units) result += su;
