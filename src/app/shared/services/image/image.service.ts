...
export class ImageService extends BaseService {
  ...
  getBreakpointsAndGutterForMasonryLayout(
    containerWidth: number,
    activeLayout: UserGalleryActiveLayout
  ): {
    ...
    if (containerWidth < Breakpoint.SM_MIN) {
      config = LAYOUT_CONFIG[activeLayout].xs;
    } else if (containerWidth < Breakpoint.MD_MIN) {
      config = LAYOUT_CONFIG[activeLayout].sm;
    } else if (containerWidth < Breakpoint.LG_MIN) {
      config = LAYOUT_CONFIG[activeLayout].md;
    } else if (containerWidth < Breakpoint.XL_MIN) {
      config = LAYOUT_CONFIG[activeLayout].lg;
    } else {
      config = LAYOUT_CONFIG[activeLayout].xl;
    }
