  ...
  implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy, OnChanges
{
  ...
  protected onSvgMouseMove(event: MouseEvent): void {
    ...
    const galacticRa = interpolationText.l
      .trim()
      .split(" ")
      .map(x => x.padStart(2, "0"));
    const galacticDec = interpolationText.b
      .trim()
      .split(" ")
      .map(x => x.padStart(2, "0"));
  ...
  private _setNonSolutionMouseHoverImage() {
    ...
    switch (this.revision.mouseHoverImage) {
      ...
      case MouseHoverImageOptions.INVERTED:
        {
          const thumbnail = this.revision.thumbnails?.find(tn => tn.alias === this.alias + "_inverted");
      ...
      case "ORIGINAL":
        {
          const thumbnail = this.image.thumbnails?.find(
            tn =>
              tn.alias === this.alias &&
              tn.revision === (this.image.isFinal ? FINAL_REVISION_LABEL : ORIGINAL_REVISION_LABEL)
          );
          this.nonSolutionMouseHoverImage = thumbnail?.url ?? null;
          this.inlineSvg = null;
        }
      ...
      default: {
        ...
        if (matchingRevision) {
          const thumbnail = matchingRevision.thumbnails?.find(tn => tn.alias === this.alias);
          this.nonSolutionMouseHoverImage = thumbnail?.url ?? null;
          this.inlineSvg = null;
        } else {
          this.nonSolutionMouseHoverImage = null;
