...
export class FullscreenImageViewerComponent extends BaseComponentDirective implements OnInit, OnChanges, OnDestroy {
  ...
  protected onTouchStart(event: TouchEvent): void {
    ...
    void this.hostElementRef.nativeElement.offsetHeight; // reflow trigger using void to avoid lint error
  ...
  protected onTouchMove(event: TouchEvent): void {
    ...
    if (wasSwipingBefore && !isSwipingNow && typeof document !== "undefined") {
      ...
      const onAnimationEnd = (animEvent: any) => {
        if (animEvent.animationName === "return-to-normal") {
  ...
  protected onTouchEnd(): void {
    ...
    if (thresholdMet) {
      ...
      const onAnimationEnd = (animEvent: any) => {
        if (animEvent.animationName === "swipe-to-close" && !animationHandled) {
  ...
  private _deregisterZoomEvents(): void {
    ...
    (this.ngxImageZoom as any).zoomInstance.onMouseMove = () => {
  ...
  private _initImageZoom() {
    if (this.ngxImageZoom) {
      ...
      (this.ngxImageZoom as any).zoomInstance.onMouseLeave = () => {
        // Preventing the default behavior to avoid jarring zoom resets
      };
    ...
    this._imageSubscription = this.store$
      ...
      .subscribe(() => {
        ...
        if (this.isGif) {
          ...
          this.imageService
            .loadImageFile(this._revision.imageFile, (progress: number) => {
