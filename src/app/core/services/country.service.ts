      ...
      .pipe(
        map((response: any) => {
          if (response !== null && response[0] !== null && response[0].region !== null) {
            let continentValue = response[0].region;

            if (continentValue === "Americas") {
              continentValue = response[0].subregion;
            }

            this.cachedContinentsMap.set(countryCode, continentValue);
