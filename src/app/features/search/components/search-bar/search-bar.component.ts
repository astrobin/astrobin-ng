...
import type { AfterViewInit, ComponentRef, OnChanges, OnDestroy, OnInit, SimpleChanges, Type } from "@angular/core";
      ...
      .subscribe(value => {
        ...
        if (value && value.length > 0) {
          ...
          from(this.searchService.autoCompleteMethods(query?.value))
            .pipe(
              concatMap(filterMethod => filterMethod.method.pipe(map(result => ({ key: filterMethod.key, result })))),
              takeUntil(this.destroyed$)
            )
            .subscribe({
              next: ({ key, result }) => {
    ...
    if (
      ...
      forkJoin(
        this.searchService
          ...
          .filter(filterItem => filterItem.key !== SearchAutoCompleteType.TEXT)
          .map(filterItem => filterItem.method)
